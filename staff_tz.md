...
2. **REST API Server** (Node.js)  
3. **PostgreSQL Database**  
4. **C\# Agent** (Windows Service)  
5. **HIKVision Integration Layer**

**3\. TIZIM ARXITEKTURASI**

HIKVision Devices ←→ API Server ←→ PostgreSQL  
                         ↕  
                    Web Interface  
                         ↕  
                   C\# Agent (Computers)  
**4\. BACKEND TALABLARI (Node.js)**

**4.1 Asosiy modullar**

* **Authentication/Authorization** \- JWT token \+ RBAC  
...
**4.2.1 Role Check Middleware**

| const checkRole \= (allowedRoles) \=\> {  return (req, res, next) \=\> {    const userRole \= req.user.role;    if (allowedRoles.includes(userRole)) {      next();    } else {      return res.status(403).json({        success: false,        error: { code: 'ACCESS\_DENIED', message: 'Ruxsat yo\\'q' }      });    }  };}; |
| :---- |

**4.2.2 Organization Scope Check**

| const checkOrganizationAccess \= async (req, res, next) \=\> {  const { role, organization\_id } \= req.user;  const targetOrgId \= req.params.organization\_id || req.body.organization\_id;    if (role \=== 'admin') {    return next(); *// Admin has access to all*  }    if (role \=== 'hr' && organization\_id \=== targetOrgId) {    return next(); *// HR can access own organization*  }    return res.status(403).json({    success: false,    error: { code: 'ORGANIZATION\_ACCESS\_DENIED', message: 'Bu tashkilotga ruxsat yo\\'q' }  });}; |
| :---- |

**4.2.3 Department Scope Check**

| const checkDepartmentAccess \= async (req, res, next) \=\> {  const { role, department\_id, sub\_department\_id } \= req.user;  const targetDeptId \= req.params.department\_id;    if (role \=== 'admin') return next();  if (role \=== 'hr') return next(); *// HR can access departments in their org*    if (role \=== 'department\_lead') {    if (department\_id \=== targetDeptId || sub\_department\_id \=== targetDeptId) {      return next();    }  }    return res.status(403).json({    success: false,    error: { code: 'DEPARTMENT\_ACCESS\_DENIED', message: 'Bu departamentga ruxsat yo\\'q' }  });}; |
| :---- |

**4.3 API Endpointlari (Role-based)**
...
**4.3.2 Organizations (Admin, HR-own)**
...
**4.3.3 Departments (Admin, HR-own org, Lead-own dept)**
...
**4.3.4 Employees (Role-based access)**
...
**4.3.5 Entry/Exit Logs (Admin, HR-own org, Lead-own dept, Guard-all)**

GET    /api/entry-logs                     \[Admin, HR-filtered, Lead-filtered, Guard\]  
POST   /api/entry-logs                     \[System only \- HIKVision\]  
...
**4.3.6 Computer Monitoring (Admin, HR-own org, Lead-own dept)**
...
POST   /api/monitoring/agent-data           \[System only \- Agent\]  
POST   /api/monitoring/register-computer-user \[System only \- Agent\]

**4.3.7 Devices (Admin only, Guard-status)**
...
**4.3.8 Visitors (Role-based access)**
...
**4.3.9 Reports (Role-based filtered)**
...
**4.3.10 Policies (Admin, HR-own org)**
...
**4.3.11 Users Management (Admin only)**
...
* **ORM**: Sequelize yoki TypeORM  
* **Connection Pool**: Maksimal 20 connection  
* **Migrations**: Database schema o'zgarishlarini boshqarish
...
**5\. FRONTEND TALABLARI (React.js)**

**5.1 UI/UX Framework**

* **UI Library**: Material-UI yoki Ant Design  
...
**5.2.1 Navigation Menu (Role-based)**

| const NavigationMenu \= ({ userRole }) \=\> {  const menuItems \= {    admin: \['Dashboard', 'Organizations', 'Users', 'Devices', 'Reports', 'Settings'\],    hr: \['Dashboard', 'Employees', 'Visitors', 'Reports', 'Monitoring'\],    department\_lead: \['Dashboard', 'My Team', 'Reports', 'Monitoring'\],    guard: \['Dashboard', 'Entry/Exit Logs', 'Current Status'\]  };    return (    \<Menu items\={menuItems\[userRole\]} /\>  );}; |
| :---- |

**5.2.2 Permission-based Components**

| const PermissionGuard \= ({ role, allowedRoles, children }) \=\> {  if (allowedRoles.includes(role)) {    return children;  }  return \<AccessDenied /\>;};// Usage\<PermissionGuard role\={user.role} allowedRoles\={\['admin', 'hr'\]}\>  \<CreateEmployeeButton /\>\</PermissionGuard\> |
| :---- |

**5.3 Sahifalar va Komponentlar (Role-based)**
...
* Umumiy statistika  
* Bugungi entry/exit grafigi  
* Hodimlar holati (ishda/yo'q)  
...
* Sub-departmentlar boshqaruvi

**4.2.4 Employee Management**

* Hodimlar ro'yxati  
* Hodim qo'shish/tahrirlash  
* Hodim kartalarini boshqarish  
* Hodim avtomobillarini boshqarish

**4.2.5 Computer Management**

* Computer Users ro'yxati (employee bilan bog'lanmagan)  
* Employee va Computer User linking  
* Computer va User mapping  
* Computer activity monitoring per employee

**4.2.6 Monitoring Pages**

* Entry/Exit logs  
* Employee computer activity (barcha linked computer users)  
* Computer-wise activity  
* Screenshots ko'rish  
* Visited websites  
* Active windows  
...
* HIKVision qurilmalar ro'yxati  
* Qurilma sozlamalari  
* Qurilma holati monitoring

**4.2.7 Visitors Management**

* Mehmonlar ro'yxati  
* Bir martalik kodlar yaratish  
* Mehmonlar entry/exit logs

**4.2.8 Reports**

* Davomat hisobotlari  
* Samaradorlik hisobotlari  
* Qurilmalar foydalanish hisoboti  
* Custom hisobotlar
...
**5.4 Data Filtering Logic**

**5.4.1 Organization Filtering**

| const useOrganizationFilter \= () \=\> {  const { user } \= useAuth();    const getOrganizationFilter \= () \=\> {    switch(user.role) {      case 'admin':        return {}; *// No filter \- can see all*      case 'hr':        return { organization\_id: user.organization\_id };      case 'department\_lead':        return { organization\_id: user.organization\_id };      case 'guard':        return { organization\_id: user.organization\_id };      default:        return { id: \-1 }; *// No access*    }  };    return getOrganizationFilter();}; |
| :---- |

**5.4.2 Department Filtering**

| const useDepartmentFilter \= () \=\> {  const { user } \= useAuth();    const getDepartmentFilter \= () \=\> {    switch(user.role) {      case 'admin':        return {}; *// All departments*      case 'hr':        return { organization\_id: user.organization\_id };      case 'department\_lead':        return {           $or: \[            { id: user.department\_id },            { id: user.sub\_department\_id }          \]        };      default:        return { id: \-1 }; *// No access*    }  };    return getDepartmentFilter();}; |
| :---- |

**5.5 Responsive Design**

* Mobile-first approach  
* Tablet va desktop uchun optimizatsiya  
...
* Windows Service sifatida ishlaydigan dastur  
* Automatic startup  
* Error handling va logging

**6.2 Monitoring Functions**

* **Active Window Tracking** \- Faol oyna kuzatuvi  
* **Website Monitoring** \- Brauzer activity kuzatuvi  
* **Screenshot Capture** \- Ekran surati olish  
* **User Session Tracking** \- Login/logout kuzatuvi

**6.3 Data Transmission**

* API Server bilan HTTP/HTTPS orqali aloqa  
* JSON formatida ma'lumot yuborish  
* Retry mechanism \- muvaffaqiyatsizlik holatida qayta urinish  
* Local buffer \- internetga ulanishda muammo bo'lganda

**6.4 Agent Registration**

* Agent ishga tushganda computer va user ma'lumotlarini yuboradi  
* SID (Security Identifier) va Computer ID ni API serverga yuboradi  
* Server yangi computer\_user yozuvini yaratadi  
* Admin/HR kerakli employeega bog'laydi

**6.5 Data Collection**

* SID va Computer ID har bir monitoring ma'lumoti bilan yuboriladi  
* Server computer\_user orqali employee ni aniqlaydi  
* Ma'lumotlar employee ga bog'lab saqlanadi

**6.6 Configuration**

* Config file orqali sozlanishi  
* API endpoint, interval settings  
* Security settings

**6.7 Agent Installation**

* MSI installer  
* Silent installation option  
* Automatic updates mechanism

**7\. HIKVision INTEGRATION**
...
| axios.interceptors.response.use(  (response) \=\> response,  async (error) \=\> {    if (error.response?.status \=== 401) {      try {        const refreshToken \= localStorage.getItem('refresh\_token');        const response \= await axios.post('/api/auth/refresh-token', {          refresh\_token: refreshToken        });                const { access\_token, refresh\_token } \= response.data.data;        localStorage.setItem('access\_token', access\_token);        localStorage.setItem('refresh\_token', refresh\_token);                *// Retry original request*        return axios(error.config);      } catch (refreshError) {        *// Redirect to login*        window.location.href \= '/login';      }    }    return Promise.reject(error);  }); |
| :---- |
...
**Phase 1: Core System \+ RBAC (1 hafta)**

* Authentication va JWT system  
* Role-based authorization middleware  
* Basic CRUD operations with permission checks  
* Database setup va migrations  
* Basic UI components with role-based navigation

**Phase 2: HIKVision Integration (3 kun)**
...
**Phase 3: Computer User Management (1 hafta)**

* Computer Users CRUD operations  
* Employee-Computer User linking interface  
* Unlinked users detection va management

**Phase 4: Agent Development (2 hafta)**

* C\# agent dasturi yaratish  
* Computer monitoring functions  
* SID va Computer ID detection  
* API integration va registration  
...
**Phase 5: Testing & Deployment (1 hafta)**
...
**12\. DEPLOYMENT VA MAINTENANCE**

**12.1 Server Requirements**

* **OS**: Ubuntu 20.04+ yoki Windows Server 2019+  
* **CPU**: 4 core minimum  
* **RAM**: 8GB minimum  
* **Storage**: 500GB SSD  
